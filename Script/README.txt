...
※ 子程式局域變量
# 默認情况下，Perl 中所有的變量都是全域變量，這就是說變量量在程式的任何地方都可以調用。
# 如果我们需要設置局域變量，可以使用 my 操作符来設置。
# my 操作符用於創建局域變量，通過 my 創建的變量，存在於宣告在各域一開始的地方，直到作用域的結束。
...
// ------------------------------Perl 引用------------------------------ //
引用就是pointer，Perl 引用是一個標量類型可以指向變量、陣列、Hash table（也叫關聯陣列）甚至子程式，可以應用在程式的任何地方。
※ 創建引用
# 定義變量的时候，在變量名前面加個\，就得到了這個變量量的一個引用
...
// -------------------------Perl 格式化輸出------------------------------- //
Perl 中可以使用 format 来定義一個模板，然後使用 write 按指定模板輸出數據。

# Perl 格式化定義與法格式如下：
format FormatName =                 # FormatName : 格式化名稱。
fieldline                           # fieldline  : 一個格式行，用@,^,<,>,| 這樣的字符，來定義一個輸出行的格式。
...
# 格式行語法
1. 格式行以 @ 或者 ^ 開頭，這些行不作任何形式的變量代換。
2. @ 字段(不要與陣列符號 @ 相混淆)是普通的字段。
3. @,^ 后的 <, >,| 長度决定了字段的長度，如果變量超出定義的長度,那麼它將被截斷。
4. <, >,| 還分别表示,左對齊,右對齊,居中對齊。
5. ^ 字段用於多行文本塊填充。

※ 備註:雖然 format 可以實現格式化輸出，但是它在現代Perl代碼中很少見，因為它的使用複雜，
而且其他方式更加靈活和易於理解，因此，建議在新代碼中使用 printf 或其他模塊進行格式化輸出。

// -------------------------Perl 文件操作------------------------------- //
※ open 函數
# 以下 open 函數範例:
open(DATA, "<file.txt") or die "file.txt 文件無法打開, $!";     # 以read-only的方式 < 打開文件 file.txt
...
# 文件處理方式
1. <  : 表示以read-only的方式打開文件
2. >  : 表示以write-only的方式打開文件
3. +< : 表示以read-write的方式打開文件
4. >> : 表示以向現有文件的尾部追加數據的方式打開文件
5. +>>: 表示以可以讀取追加數據的方式打開文件
